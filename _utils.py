...
def train_val_split(eps_data: typing.List[torch.Tensor], k_shot: int) -> typing.Dict[str, torch.Tensor]:
    ...
    data = {
        'x_t': torch.empty(size=(num_classes, k_shot, iH), device=eps_data[0].device),
        'x_v': torch.empty(size=(num_classes, v_shot, iH), device=eps_data[0].device),
    ...
    for cls_id in range(num_classes):
        X = np.squeeze(eps_data[0])[labels == cls_id]
        # print(X.shape)
    ...
    data['x_t'] = data['x_t'].view(num_classes * k_shot, iH)
    data['x_v'] = data['x_v'].view(num_classes * v_shot, iH)
