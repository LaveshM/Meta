...
class Vampire2(MLBaseClass):
    ...
    def evaluation(self, x_t: torch.Tensor, y_t: torch.Tensor, x_v: torch.Tensor, y_v: torch.Tensor, model: dict) -> typing.Tuple[float, float]:
        ...
        logitsr = self.prediction(x=xr, adapted_hyper_net=adapted_hyper_net, model=model)
        y_predr = 0
        
        for logits_ in logitsr:
            y_predr = y_predr + torch.softmax(input=logits_, dim=1)
        
        y_predr = y_predr / len(logitsr)

        # convert logits to probabilities
        probs = torch.softmax(y_predr, dim=1)
